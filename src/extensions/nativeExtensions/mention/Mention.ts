import { CustomMention as MentionOriginal } from './CustomMention'
...
export default class Mention extends AbstractExtension {
  constructor (options) {
    if (!options.hasOwnProperty('suggestion')) {
  ...
  get availableActions (): ExtensionActionInterface[] {
    const nativeExtensionName = 'customMention'

    return [
      {
        render: new ExtensionActionRenderBtn({
          tooltip: new I18nText('extensions.Mention.buttons.tooltip'),
          icons: {
            [VuetifyIconsGroups.md]: new VuetifyIcon('at'),
          ...
          async onClick ({ editor }) {
            ...
            let text = '@'
            if (typeof before === 'string' && before.slice(-1) !== ' ') text = ' ' + text
            editor.chain().focus().insertContent({ type: 'text', text: text }).run()
